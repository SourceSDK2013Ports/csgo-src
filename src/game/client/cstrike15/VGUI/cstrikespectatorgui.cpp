...
DECLARE_HUD_MESSAGE( CCSMapOverview, ProcessSpottedEntityUpdate )
...
CCSMapOverview::CCSMapOverview( const char *pElementName ) : BaseClass( pElementName )
{
	...
	SetMapOverView( GET_ACTIVE_SPLITSCREEN_SLOT(), this ); // for cvars access etc
...
void CCSMapOverview::Init( void )
{
	...
	HOOK_HUD_MESSAGE( CCSMapOverview, ProcessSpottedEntityUpdate );
}
 ...
 */
bool CCSMapOverview::MsgFunc_ProcessSpottedEntityUpdate( const CCSUsrMsg_ProcessSpottedEntityUpdate &msg )
{
	if ( msg.new_update() )
	{
	...
	for ( int i = 0; i < msg.entity_updates_size(); i ++ )
	{
		...
		{
			MapPlayer_t* player = GetPlayerByEntityID( nEntityID - 1 );
			if ( !player )
				continue;
			...
			player->position.Init( x, y, z );
			player->angle.Init( 0, a, 0 );
		...
		{
			MapPlayer_t* hostage = GetHostageByEntityID( nEntityID );
			if ( hostage )
				hostage->position.Init( x, y, z );
		}
		else
		{
...
void CCSMapOverview::UpdatePlayers()
{
	...
	if ( GetMode() == MAP_MODE_RADAR )
	{
		...
		for ( int i = 1; i<= gpGlobals->maxClients; ++i )
		{
			...
			MapPlayer_t *player = &m_Players[i-1];
			if ( m_EntitySpotted.Get(player->index+1) )
			{
				SetPlayerSeen( i-1 );
